...
describe('Node Integration Tests', () => {
  ...
  describe('Single Node', () => {
    test('should start a single node successfully', async () => {
      const port = 13000;
      const node = await Node.create(port, 'localhost');
      nodes.push(node);
  ...
  describe('Two Nodes - Peer Discovery', () => {
    test('should establish bidirectional connection between two nodes', async () => {
      ...
      // Start first node
      const node1 = await Node.create(port1, 'localhost');
      nodes.push(node1);
      await node1.start();
      ...
      // Start second node
      const node2 = await Node.create(port2, 'localhost');
      nodes.push(node2);
      await node2.start();
    ...
    test('should propagate transactions between two connected nodes', async () => {
      ...
      await node1.addTransaction(node1Info.address, 'recipient_address', 100);
  ...
  describe('Three Nodes - Transitive Discovery', () => {
    test('should enable transitive peer discovery (A->B, C->B results in A<->C)', async () => {
      ...
      // Start node B (hub)
      const nodeB = await Node.create(portB, 'localhost');
      nodes.push(nodeB);
      await nodeB.start();
      ...
      const nodeA = await Node.create(portA, 'localhost');
      nodes.push(nodeA);
      await nodeA.start();
      ...
      const nodeC = await Node.create(portC, 'localhost');
      nodes.push(nodeC);
      await nodeC.start();
    ...
    test('should propagate transactions across three connected nodes', async () => {
      ...
      await nodeA.addTransaction(nodeAInfo.address, 'recipient_xyz', 50);
    ...
    test('should maintain network connectivity when middle node receives transaction', async () => {
      ...
      await nodeB.addTransaction(nodeBInfo.address, 'recipient_hub', 75);
